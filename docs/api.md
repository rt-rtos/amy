...
| `C`    | `dist_type`, `dist_drive`, `dist_bits`, `dist_rate`, `dist_mix` | `dist` | list of 5 floats | Per-oscillator distortion, applied before the filter, as [type, drive, bits, rate, mix]. type: 0 = off (default), 1 = soft clip, 2 = wavefold, 3 = bitcrush. drive: 0-16 pre-gain (fold depth for wavefold). bits: bitcrush bit depth, >= 24 leaves bit depth unchanged. rate: bitcrush sample-hold length in samples, 1 disables. mix: 0-1 wet/dry. |
